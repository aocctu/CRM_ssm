...
function removeConfirm(){
    ...
    if (row){
    	$.messager.confirm('信息提示', '确认要删除吗?', function(r){
            if (r){
            	$.get(contextPath+"/expressCompany/delete.do?id="+row.id,null,
					function(data,state){
						if(data.status==1){
...
function loadRemote(){
	var row = $('#dg').datagrid('getSelected');
    if (row){
        $('#editForm').form('load', contextPath+'/expressCompany/update.do?id='+row.id);
	...
	$("#add-save-button").click(
		function(){
			...
			$.messager.confirm('信息提示', '确认要保存吗?', function(r){
   	            if (r){
   	            	$.post(contextPath+"/expressCompany/addSave.do",datas,
						function(data,state){
							if(data.status==1){
...
$(function(){//保存修改
	$("#edit-save-button").click(
		function(){
			...
			$.messager.confirm('信息提示', '确认要保存吗?', function(r){
   	            if (r){
   	            	$.post(contextPath+"/expressCompany/updateSave.do",datas,
						function(data,state){
							if(data.status==1){
								$.messager.alert('信息提示','保存成功');
...
$(function(){
    ...
    $('#dg').datagrid({
    	columns:[[
			...
			{field:'exp_company',title:'快递公司', width:160},
    	]]
    });
