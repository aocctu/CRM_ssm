...
function removeConfirm(){
    ...
    if (row){
    	$.messager.confirm('信息提示', '确认要删除吗?', function(r){
            if (r){
            	$.get(contextPath+"/resetpass/delete.do?id="+row.id,null,
					function(data,state){
						if(data.status==1){
...
function loadRemote(){
	var row = $('#dg').datagrid('getSelected');
    if (row){
        $('#editForm').form('load', contextPath+'/resetpass/update.do?id='+row.id);
		if(row.resetpass){
			$("#edit_parent_id").combobox('select', row.resetpass.id);
		}
	...
	$("#add-save-button").click(
		function(){
			...
			$.messager.confirm('信息提示', '确认要保存吗?', function(r){
   	            if (r){
   	            	$.post(contextPath+"/resetpass/addSave.do",datas,
						function(data,state){
							if(data.status==1){
...
$(function(){//保存修改
	$("#edit-save-button").click(
		function(){
			...
			$.messager.confirm('信息提示', '确认要保存吗?', function(r){
   	            if (r){
   	            	$.post(contextPath+"/resetpass/updateSave.do",datas,
						function(data,state){
							if(data.status==1){
								$.messager.alert('信息提示','保存成功');
...
$(function(){
    ...
    $('#dg').datagrid({
    	columns:[[
			...
			{field:'phoneNo',title:'手机', width:160},
