...
function removeConfirm(){
    ...
    if (row){
    	$.messager.confirm('信息提示', '确认要删除吗?', function(r){
            if (r){
            	$.get(contextPath+"/admin/delete.do?id="+row.id,null,
					function(data,state){
						if(data.status==1){
...
function loadRemote(){
	var row = $('#dg').datagrid('getSelected');
    if (row){
        $('#editForm').form('load', contextPath+'/admin/update.do?id='+row.id);
		if(row.admin){
			$("#edit_status").combobox('select', row.status);
		}
		
		if(row.adminRoles){
			$("#edit_role_name").combobox('select', row.adminRoles.id);
		}
	...
	$("#add-save-button").click(
		function(){
			...
			$.messager.confirm('信息提示', '确认要保存吗?', function(r){
   	            if (r){
   	            	$.post(contextPath+"/admin/addSave.do",datas,
						function(data,state){
							if(data.status==1){
...
$(function(){//保存修改
	$("#edit-save-button").click(
		function(){
			...
			$.messager.confirm('信息提示', '确认要保存吗?', function(r){
   	            if (r){
   	            	$.post(contextPath+"/admin/updateSave.do",datas,
						function(data,state){
							if(data.status==1){
								$.messager.alert('信息提示','保存成功');
    ...
    $('#dg').datagrid({
    	columns:[[
    		...
    		{field:'role_name',title:'角色名称', width:160,
    			formatter: function(value,row,index){
    				if (row.adminRoles){
    					return row.adminRoles.role_name;
    				} else {
    					return value;
    				}
    			}
    		...
    		{field:'login_times',title:'登录次数', width:60},
    		{field:'create_date',title:'创建时间', width:100},
    		{field:'login_date',title:'最后登录日期', width:100},
